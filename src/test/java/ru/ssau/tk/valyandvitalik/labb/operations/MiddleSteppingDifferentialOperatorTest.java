...
public class MiddleSteppingDifferentialOperatorTest {
    ...
    public void testDerive() {
        ...
        assertEquals(differentialOperator.derive(new FunctionCube()).apply(4), (Math.pow(4 + step, 3) - Math.pow((4 - step), 3)) / (2 * step));
        assertEquals(differentialOperator.derive(new FunctionSin()).apply(45), (Math.sin(45 + step) - Math.sin(45 - step)) / (2 * step));
        assertEquals(differentialOperator.derive(new FunctionSqrt()).apply(144), (Math.sqrt(144 + step) - Math.sqrt(144 - step)) / (2 * step));
    }
}
